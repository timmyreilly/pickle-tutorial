...
const result1 = add(1, 2); // you get three immediately. 

// const result2 = getAddResultFromServer('http://www.example.com?num1=1&num2=2'); 

// async and promises aren't strictly necessary you could always use callbacks. So... a function will be called when the function is finished. 

const fetch = require('node-fetch');

function addAsyn(num3, num4, callback) {
    return https.get('http://www.example.com', {
        num1: num1,
        num3: num4
    }, callback);
}
...
addAsync(1, 2)
    ...
    // fudge... now there's new thing called observables. 

    // Observables are lazy event streams which can emit zero or more events, and may or may not finish: https://cycle.js.org/streams.html 

// Observables are cancellable and lazy 

// RxJS for observables. that's a microsoft thing. next level! 
...
adddddd(1, 2)
.do(x => rA =x)
.flatMap(x => adddddd(x, 3))
.do(x => rB =x)
.flatMap(x => adddddd(x, 4))
.do(x => rC =x)
.subscribe(x => {
    console.log('total: ', x)
